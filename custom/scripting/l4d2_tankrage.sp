...
void Event_TankSpawn(Event hEvent, char[] sEventName, bool dontBroadcast)
{
    ...
    */
    ...
    {
        PrintToChatAll("[交界地] 生还者每后退 %d%% 地图进度，Tank 怒气冻结 %.1f 秒。", convarRageFlowPercent.IntValue, convarRageFreezeTime.FloatValue);
...
{
    if (iTank > 0 && iTank <= MaxClients && IsClientInGame(iTank) && !IsFakeClient(iTank))
    {
        ...
        if (flowPercent <= 0) return Plugin_Continue;
        ...
        if (diff >= flowPercent)
        {
            ...
            PrintToChatAll("[交界地] 生还者后退触发：Tank 怒气冻结 +%.1f 秒。", fTimeToAdd);
            LogMessage("JJD_RAGE_FREEZE added=%.1f tank=%d", fTimeToAdd, GetClientUserId(iTank));
        }
    }
