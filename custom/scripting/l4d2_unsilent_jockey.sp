...
{
	// ConVars
	g_hJockeyVoiceInterval = CreateConVar("sm_unsilentjockey_interval", "2.0", "Interval between forced jockey sounds.", _, true, 0.5, true, 10.0);
...
void PlayerSpawn_Event(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (client > 0) RequestFrame(JockeyRideEnd_NextFrame, GetClientUserId(client));
}
...
bool ValidJockey(int client)
{
    return client > 0 && client <= MaxClients && IsClientInGame(client) && IsPlayerAlive(client)
        && GetClientTeam(client) == TEAM_INFECTED && GetEntProp(client, Prop_Send, "m_zombieClass") == ZC_JOCKEY
        && !GetEntProp(client, Prop_Send, "m_isGhost") && GetEntPropEnt(client, Prop_Send, "m_jockeyVictim") <= 0;
}

void PlayJockey(int client)
{
    int pick = GetRandomInt(0, sizeof(g_sJockeySound) - 1);
    EmitSoundToAll(g_sJockeySound[pick], client, SNDCHAN_VOICE, SNDLEVEL_HELICOPTER);
}

Action delayedJockeySound(Handle timer, any userid)
{
    int client = GetClientOfUserId(userid);
    if (!ValidJockey(client)) {
        if (client > 0) g_hJockeySoundTimer[client] = null;
        return Plugin_Stop;
    }
    PlayJockey(client);
...
void ChangeJockeyTimerStatus(int client, bool enable)
{
    if (client < 1 || client > MaxClients) return;
    delete g_hJockeySoundTimer[client];
    if (enable && ValidJockey(client)) {
        PlayJockey(client); // First cue immediately after materialization / ride end.
        g_hJockeySoundTimer[client] = CreateTimer(g_hJockeyVoiceInterval.FloatValue, delayedJockeySound,
            GetClientUserId(client), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
    }
}
public void OnClientDisconnect(int client) { ChangeJockeyTimerStatus(client, false); }
public void OnMapEnd() { for (int c=1;c<=MaxClients;c++) ChangeJockeyTimerStatus(c, false); }
