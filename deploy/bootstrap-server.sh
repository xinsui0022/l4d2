#!/usr/bin/env bash
# New Ubuntu 22.04 machine only. Run as root: bash bootstrap-server.sh GAME_USER
set -euo pipefail
test "$(id -u)" = 0
...
apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y curl ca-certificates python3 sqlite3 nginx lib32gcc-s1 lib32stdc++6 libc6:i386 libstdc++6:i386 libcurl3-gnutls:i386
...
echo "Base installed. Install your SSH public key for $game_user, log in as that user and run restore-server.py. Allow cloud firewall UDP 27015, TCP 80 and your SSH port."
